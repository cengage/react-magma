...
- `src/index.ts` re-exports three component groups: the legacy `./components/LineChart`, `./components/CarbonChart`, and `./components/ChartTable`.
...
- **Build**: uses **Rollup** (`rollup.config.js`) to emit ESM, CJS, and UMD bundles, process CSS through PostCSS injection, and externalize dependencies. `react-magma-dom` uses TSDX; `dropzone` uses microbundle.
...
- `DropzoneProps` extends `FormFieldContainerBaseProps` and exposes file constraints, drag behavior, upload lifecycle callbacks, thumbnail handling, and a `dropzoneOptions` pass-through to `react-dropzone`, among other form-field props. **Note**: a code comment states these props are manually copied into `dropzone.mdx` — the docs page for this component is hand-maintained, not derived from `properties.json` the way `react-magma-dom` components are.
