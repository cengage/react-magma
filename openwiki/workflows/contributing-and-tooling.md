...
- **npm workspaces** (`package.json`): `workspaces: ["website/*", "packages/*", "tests/*"]`.
- **Lerna** (`lerna.json`) runs cross-package scripts and handles independent semantic versioning (`"version": "independent"`). Its legacy `lerna publish` allow-list contains `dev`, `main`, and `hotfix/*`; the stable v4 workflow publishes with Changesets instead.
- **Nx** (`nx.json`) caches `build` and declares build dependencies — e.g. `charts` and `dropzone` both depend on `react-magma-dom:build`.
- Root `package.json` scripts wrap these tools: `npm run build` runs workspace builds through Lerna, while `npm run build:lite` excludes `react-magma-docs` and is what `postinstall` runs after `npm ci`.
...
- **Every change needs a changeset**, including docs, CI, dependency, and workflow-only changes: add a hand-written file at `.changeset/<descriptive-name>.md` (do not rely on the `npm run changeset` interactive prompt — write it directly per `AGENTS.md`):
  ...
  List every affected package (`react-magma-dom`, `@react-magma/charts`, `@react-magma/dropzone`, `react-magma-docs`); use `patch`/`minor`/`major` per semver impact.
...
- `.github/workflows/publish.yml` runs `version:pkgs` and then `npm run release` (`changeset publish`). The separate `publish` script (`copy:readme` + `lerna publish`) is a legacy/manual path, not the stable workflow. Stable v4 packages publish with the `v4-latest` dist-tag after the release workflow update.

## Branching, commits, PRs

- Branch from and target **`v4/dev`**, not `v4/main` or the v5 branches.
- **Conventional Commits scoped by component, package, or docs area**, e.g. `feat(TreeView): support folder expansion`, `fix(chart): add accessible chart role`, `docs(OpenWiki): correct repository guidance`, `docs(AGENTS): clarify commit rules`. Husky's `commit-msg` hook validates via commitlint; `pre-commit` runs lint-staged, including related package Jest tests for JS/TS and configured ESLint/Prettier steps — see `.lintstagedrc`, `commitlint.config.js`, `.husky/`.
...
- **`preview.yml`** — runs on PRs to `dev`/`main`/`v4/dev`/`v4/main`: `npm ci --legacy-peer-deps`, Jest, Playwright, the docs build, `npm run build:storybook`, and Netlify previews for both docs and Storybook with a v4/v5 alias derived from the target branch.
- **`publish.yml`** — versions and publishes packages, then builds and deploys the appropriate docs, Storybook, and landing artifacts for development and stable branches.
- **`deploy-v2-docs.yml`** — deploys the v2 documentation site.
...
1. `npm run lint`, `npm test`, and `npm run build` all pass clean.
