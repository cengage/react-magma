...
- **Structure**: Lerna + Nx orchestrated npm workspaces (`workspaces`: `packages/*`, `website/*`; see root [package.json](../package.json) and [nx.json](../nx.json)).
...
| `.github/workflows/` | CI: PR preview (test/build/Playwright/Chromatic/Netlify), publish, docs deploy, Snyk. | [Contributing & Tooling](workflows/contributing-and-tooling.md#ci-workflows) |
| `examples/token-demo` | Standalone consumer demo app. | — |
...
```sh
npm ci                  # install (postinstall runs npm run build:lite)
npm run storybook       # component playground at http://localhost:6006
npm run docs            # docs site at http://localhost:8000 (Gatsby)
npm test                # jest across all packages (pretest runs compiler-checks)
npm run build           # compiler-checks + lint, then build all packages
```
