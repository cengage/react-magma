...
export const NextVersionTable = () => {
  ...
  React.useEffect(() => {
    if (tags && tags.next) {
      setRow(<TableRow><TableCell>v{tags.next.version}</TableCell><TableCell>{tags.next.date}</TableCell><TableCell><a href='https://next--upbeat-sinoussi-f675aa.netlify.app/'>Docs</a></TableCell><TableCell>{tags.next.version >= '4' ? '^17.0.2' : '^16.10.0'}</TableCell></TableRow>);
    } else {
      setRow(<></>);
    }
  }, [tags]);
