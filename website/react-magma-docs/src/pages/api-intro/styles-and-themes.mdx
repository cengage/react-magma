...
<LeadParagraph>
  The Magma component library has been intentionally designed for use across
  multiple platforms. It is recommended to use the default styles that come with
  any component whenever possible. However, there may be occasions where
  overriding the default styles is required, and this can be accomplished in a
  number of ways.
</LeadParagraph>

Upgrading to the 2026 Magma theme? Review the{' '}

<Link to="/api-intro/2026-theme-migration/">2026 Theme Migration guide</Link>
for guidance on replacing legacy tokens and preserving existing styles.
...
Magma styles are built with the [Emotion library](https://emotion.sh/docs/styled) using styled-components.
If you are using Emotion in your app, you can extend the styles of Magma components. This is useful if you want to add custom styles that do not fit into the `style` prop, such as styles for pseudo elements, pseudo classes, and media queries.

Note: If the styles in your app are injected before the Magma styles, your styles might not get applied if the specificity is the same.
...
By default, all components in the Magma library use the Magma theme. You can use the ThemeContext to pass in a custom theme.
...
const magmaDark = Object.assign({}, magma, {
  colors: Object.assign({}, magma.colors, {
    primary: magma.colors.tangerine600, // link color
    focus: magma.colors.neutral0,
    danger: magma.colors.red200,
    border: magma.colors.tangerine600,
  }),
});

function DemoComponent({ isInverse }) {
  return (
    <Card isInverse={isInverse}>
      <CardBody>
        ...
        />
        <RadioGroup isInverse={isInverse} labelText="Radio Group" name="radio">
          <Radio labelText="Radio 1" value="1" />
          <Radio labelText="Radio 2" value="2" />
        </RadioGroup>
        <ButtonGroup isInverse={isInverse}>
  ...
  return (
    <div>
      ...
      <ThemeContext.Provider value={theme}>
        <DemoComponent isInverse={theme === magmaDark} />
...
To implement theming in your project, we recommend a theme file that contains a theme object that matches Magma's existing theme structure.
...
You'll have to wrap your entire app - or individual components, for more specific theme changes - in the ThemeContext provider from Magma. Any descendants of the context provider will receive the theme that is passed in. Individual
...
const springfieldDonut = Object.assign({}, magma, {
  ...
  appBar: {
    ...
    inverse: {
      backgroundColor: magma.colors.indigo600,
