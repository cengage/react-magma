...
- feat(Fieldset): Add new `Fieldset` component for grouping related form controls under a shared legend.
...
- fix(TreeView): Announce the expand/collapse state of `TreeItem` to screen readers.
...
- cab09c2ad: feat(Charts): Add accessible chart toolbar with "Show as table", fullscreen, and "More options" buttons
...
- e23b1a2f9: fix(Tab & NavTab): Add focus on the back and next buttons. Add announce tab content. Update aria-labels. fix(Combobox, MultiCombobox): Add defensive optional chaining on i18n .replace() calls to prevent crashes if i18n values are undefined.
...
- 1bd563f: feat(DefinitionList): New `DefinitionList` and `DefinitionListItem` components.
  @deprecated List `description` prop
- e9f3837: feat(Input): Add new types: `tel` and `url`.
- edbc5c0: feat(TreeView): Add `expandAll` and `collapseAll`. Removed collapse/expand animations for `TreeView`.
- a3dccc9: feat(TreeView): Retain expanded/collapsed state
- 01c8227: feat(Popover): New `Popover` component.
- 57e6076: feat(TreeView): Add 'isTopLevelSelectable' prop

### Patch Changes

- b65154a: fix(SkipLink): Fix overlap with the SkipLink component when used with AppBar.
- 403f375: fix(Dropdown): Added icon prop for `DropdownExpandableMenuListItem`
- c942de9: fix(Dropdown): Display Dropdown content above other elements.
- 1a87ef7: fix(Input): Update the label height styling for the `Input` component.
- 122eb21: fix(Modal): Delete timeout of closing modal
- 84daa4e: fix(Pagination): Changed the ellipsis characters to an HTML entity for accessibility compliance.
- 9062d6c: fix(PasswordInput): Fix webkit styles when using the password manager.
- 7dc4a29: fix(Search): Update `clear` and `search` icon styles
- 28de863: fix(SkipLink): Deprecate `to` prop.
- eeda479: fix(Tabs): Improve scroll spy functionality for smaller screens.
- 998d983: fix(Theme): Update types for theme in the components.
...
- a9ebf93: fix(Tooltip): Replace Popper with Floating UI. Change default tooltip `arrowSize` and `arrowSizeDoubled` in theme.
- 559a2ad: fix(Input): Screenreader fix for Firefox when errorMessage and helperMessage exist simultaneously.
- 21d554d: fix(Pagination): Fix support for `page` in `PaginationType.simple`
- 52f8a98: chore: Update to react-magma-icons v3.1.0
- f2ad9bd: chore: Update UUID support to include v8.3.0 - v11.1.0
...
- ede3d0f62: feat(Tabs): Two new components, `TabsScrollSpyContainer` and `TabScrollSpyPanel` allow for a unique `Tab` layout with a scroll-spy navigation UI. Please note that this component is in _beta_. Use at your own risk.
...
- 9a536bcda: feat(Modal): Add new `isModalClosingControlledManually` prop that allows handling closing the modal on the consumer side
