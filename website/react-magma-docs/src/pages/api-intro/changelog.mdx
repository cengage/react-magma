...
- edbc5c0: feat(TreeView): add expandAll and collapseAll methods
- 01c8227: feat(Popover): New Popover component
- e9f3837: feat(Input): Support new types: tel and url
- 1bd563f: feat(DefinitionList): Introduce DefinitionList и DefinitionListItem компоненты
- 4f77343: feat(Popover): Add Esc key support, fix docs, add maxWidth option
- 57e6076: feat(TreeView): Add isTopLevelSelectable prop

### Patch Changes

- 1a87ef7: fix(Input): All inputs not the same height
- 559a2ad: fix(Input > Accessibility): Screenreader issue on Firefox when errorMessage и helperMessage coexist
- c942de9: fix(Dropdown): Content layering under other elements
- 403f375: fix(DropdownExpandableMenuListItem): Add icon prop
- c494d79: fix(package): Update @types/react to resolve type errors
- 7dc4a29: fix(Search): Style issues with clear and search icons
- ec6ed54: fix(Pagination): support page prop in SimplePagination
- 84daa4e: fix(Pagination): use HTML entity for ellipsis
- 9062d6c: fix(password input): WebKit styles when using password manager
- 28de863: fix(SkipLink): Deprecate to prop
- b65154a: fix(SkipLink): overlap with AppBar
- 221a7c9: fix(SkipLink): Remove to prop
- 8b24132: fix(Dropzone): Fix TS path
- 21974cf: fix(DefinitionList): Fix Sandbox errors
- a3dccc9: fix(TreeView): Retain expanded state on keyboard navigation
- eeda479: fix(Tabs): Improve scroll on smaller screens
- a9ebf93: fix(tooltip): Replace popper with floating-ui
- 122eb21: fix(Modal): Closing modals is slow
