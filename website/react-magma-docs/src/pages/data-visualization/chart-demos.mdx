...
Every example below passes `chartToolbar={{}}` to `CarbonChart`. This opts the chart into the accessible Magma toolbar ("Show as table", fullscreen, and "More options" with CSV/PNG/JPG downloads), and replaces Carbon's inaccessible `<p role="heading">` chart title with a semantic heading read from `options.title`. The default heading level is `<h2>`. See the [Toolbar customization](#toolbar_customization) section below for advanced options, and [Getting Started](../introduction#accessible_chart_toolbar) for the underlying accessibility rationale.
...
The demos above use the default toolbar (`chartToolbar={{}}`). The `chartToolbar` object accepts options for adding menu items, customizing the data table, setting the chart title heading level, hiding individual buttons, and theming. See [Getting Started](../introduction#chart_toolbar_props) for the full props reference and the list of standalone components exported from `@react-magma/charts`.
...
`beforeTitle` and `afterTitle` let you render custom content inline with the built-in chart title. They are the chart equivalent of the `additionalContent` prop on [Input](../../api/input#help_link), and the same guidance applies: they work best with compact inline elements such as a `Badge` or a `Tooltip` wrapping an `IconButton`, with the button's `aria-label` matching the tooltip text.
...
export function Example() {
  const infoLabel = 'Scores are averaged across all attempts in the term.';
