...
Every example below passes `chartToolbar={{}}` to `CarbonChart`. This opts the chart into the accessible Magma toolbar ("Show as table", fullscreen, and "More options" with CSV/PNG/JPG downloads), and replaces Carbon's inaccessible `<p role="heading">` chart title with a semantic `<h2>` read from `options.title`. See the [Toolbar customization](#toolbar-customization) section below for advanced options, and [Getting Started](../introduction#accessible_chart_toolbar) for the underlying accessibility rationale.
