...
- The chart title uses `<p role="heading">` instead of a semantic heading element
...
<SimplePropsTable
  propertyValues={{
    ...
    titleLevel: {
      ...
      required: false,
      description:
        'Heading level used for the chart title rendered from options.title.',
      defaultValue: '2',
    },
    tableColumns: {
...
- **1.3.1 Info and Relationships (Level A)** — Chart title uses a semantic `<h1>`–`<h6>` (default `<h2>`) instead of `<p role="heading">`. Modal has a semantic heading. Table uses `<th>` elements.
