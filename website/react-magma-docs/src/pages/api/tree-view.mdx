...
## Check All Children / Check all parents when children are checked

```typescript
import React from 'react';
import { TreeView, TreeItem, magma, TreeViewSelectable } from 'react-magma-dom';
...
export function Example() {
  ...
  return (
    <>
      <Checkbox
        labelText="Check All Children on Parent Select"
        onChange={changeCheckChildren}
        checked={checkChildren}
      />
      <Checkbox
        labelText="Check Parents on children select"
