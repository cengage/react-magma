...
export function Example() {
  return (
    <TreeView
      ariaLabelledBy="istoplevel-animals-multi"
      selectable={TreeViewSelectable.multi}
      isTopLevelSelectable={false}
    >
      <TreeItem label="Birds" itemId="istoplevel-birds">
        <TreeItem label="Parrots" itemId="istoplevel-parrots">
          <TreeItem
            label="African Grey"
            isDisabled
            itemId="istoplevel-african-grey"
          />
          <TreeItem label="Cockatiel" itemId="istoplevel-cockatiel" />
          <TreeItem label="Budgerigar" itemId="istoplevel-budgerigar" />
        </TreeItem>
        <TreeItem label="Birds of Prey" itemId="istoplevel-birds-of-prey">
          <TreeItem label="Eagles" itemId="istoplevel-eagles" />
          <TreeItem label="Hawks" itemId="istoplevel-hawks" />
          <TreeItem label="Falcons" itemId="istoplevel-falcons" />
        </TreeItem>
      </TreeItem>
      <TreeItem label="Aquatic Animals" itemId="istoplevel-aquatic-animals">
        <TreeItem label="Fish" itemId="istoplevel-fish">
          <TreeItem label="Goldfish" itemId="istoplevel-goldfish" />
          <TreeItem label="Betta Fish" itemId="istoplevel-betta-fish" />
          <TreeItem label="Guppies" itemId="istoplevel-guppies" />
        </TreeItem>
      </TreeItem>
      <TreeItem label="Marine Mammals" itemId="istoplevel-marine-mammals">
        <TreeItem label="Dolphins" isDisabled itemId="istoplevel-dolphins" />
        <TreeItem label="Whales" isDisabled itemId="istoplevel-whales" />
        <TreeItem label="Seals" isDisabled itemId="istoplevel-seals" />
      </TreeItem>
      <TreeItem label="Land Mammals" itemId="istoplevel-land-mammals" />
    </TreeView>
  );
}
```
...
## Dynamically Add TreeItem

Use the `apiRef.addItem(item)` prop to add a new item. This function accepts an item of type `TreeViewItemInterface`. If the item includes a `parentId`, it is added as a **child**; otherwise, it is added as a **parent**.
...
export function Example() {
  const treeContent = {
    ...
    items: [
      {
        id: 'dynamic-discipline-arts-design',
        title: 'Arts and Design',
        children: [
          {
            id: 'dynamic-ad-1',
      ...
      {
        id: 'dynamic-discipline-business',
        title: 'Business',
        children: [
          {
            id: 'dynamic-bsn-1',
      ...
      {
        id: 'dynamic-discipline-geography',
        title: 'Geography',
        children: [],
      },
    ],
    preselectedItems: [
      {
        itemId: 'dynamic-bsn-1',
        checkedStatus: IndeterminateCheckboxStatus.checked,
      },
      {
        itemId: 'dynamic-nutr-1',
        checkedStatus: IndeterminateCheckboxStatus.checked,
      },
    ],
  };
  ...
  const apiRef = React.useRef<TreeViewApi>();
  const [, setSelectedItems] = React.useState<TreeItemSelectedInterface[]>();
  const [items, setItems] = React.useState(treeContent.items);
  ...
  const renderTreeItemsRecursively = (discipline: any[], depth: number) => {
    return discipline.map(term => {
      const isDisabled =
        term.title === 'Accounting' || term.title === 'Computer Science';
  ...
  const toggleAddNewParent = () => {
    const treeLength = items.length + 1;
    const newItemId = 'new-parent-' + treeLength;
      ...
      if (index === items.length - 1) {
        return {
          ...
          children: [
            ...
            {
              id: newItemId,
              title: `New ${item.title} Child ${item.children.length + 1}`,
              children: [],
            },
          ],
        };
  ...
  return (
    <>
      <ButtonGroup
        ...
        color={ButtonColor.subtle}
      >
        <Button onClick={toggleAddNewParent}>Add parent</Button>
        <Button onClick={toggleAddNewChild}>
      ...
      <TreeView
        ...
        apiRef={apiRef}
      >
        {renderTreeItemsRecursively(items, 0)}
      </TreeView>
...
When `hasDivider` is set to `true`, a vertical line is rendered alongside expanded parent items to visually connect them with their children. This helps users understand the hierarchy at a glance, especially in deep or complex trees.

By default, `hasDivider` is set to `false`.
