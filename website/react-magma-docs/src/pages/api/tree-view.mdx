...
Use the `apiRef.showAll()` prop when using a Show All / Show Less button inside your tree.

```typescript
import React from 'react';
import { TreeView, TreeItem, TreeViewSelectable, TreeItemSelectedInterface, Spacer, SpacerAxis, IconButton, ButtonSize, ButtonVariant } from 'react-magma-dom';
