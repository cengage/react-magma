...
Use the `apiRef.addNewItem(item)` prop to add a new item. This function accepts an item of type `TreeViewItemInterface`. If the item includes a `parentId`, it is added as a **child**; otherwise, it is added as a **parent**.
