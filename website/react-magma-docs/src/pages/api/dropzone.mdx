...
<Alert variant="warning"><strong>Important!</strong> Dropzone is imported from <code>@react-magma/dropzone</code> which must be installed as a peer dependency.</Alert>
...
import { Dropzone, OnSendFileProps } from '@react-magma/dropzone';
import { Button, Spacer } from 'react-magma-dom';

export function Example() {
  const [sendFiles, setSendFiles] = React.useState<boolean>(false);
  const uploadFilesButtonRef = React.useRef<HTMLButtonElement>(null);
  ...
  const onSendFile = (props: OnSendFileProps) => {
    ...
    const interval = setInterval(() => {
      ...
      if (percent >= 100) {
        ...
        setTimeout(() => {
          uploadFilesButtonRef.current?.focus();
        }, 0);
