...
import { Card, CardBody, TimePicker, magma } from 'react-magma-dom';
...
import { Button, TimePicker, Announce, VisuallyHidden } from 'react-magma-dom';

export function Example() {
  const [timeValue, setTimeValue] = React.useState<string | undefined>('');
  const [announcement, setAnnouncement] = React.useState('');
  const [isCleared, setIsCleared] = React.useState(false);
  const clearedText = 'Time has been cleared';

  function handleOnChange(value) {
    setAnnouncement(isCleared ? clearedText : '');
    setTimeValue(value);
    setIsCleared(false);
  }

  function onButtonClick() {
    setIsCleared(timeValue ? true : false);
  ...
  return (
    <>
      ...
      />
      <br />
      <Button onClick={onButtonClick}>Clear Time</Button>
      <Announce>
...
export function Example() {
  return (
    <TimePicker
      ...
      messageStyle={{ background: '#0000C7', color: '#FFFFFF' }}
