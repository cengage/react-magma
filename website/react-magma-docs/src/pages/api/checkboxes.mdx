...
<br />
See also: <Link to="/api/radio">Radio Buttons</Link> and
<Link to="/api/toggle">Toggle Switches</Link>.
...
<Alert variant="warning">
  ...
  only be used in rare circumstances when the purpose of the checkboxis conveyed
  in another way. While screen readers will announce the label when the checkbox
...
- **Mind accessibility:** virtualization removes off-screen checkboxes from the
  DOM, so screen readers can't reach them — a real trade-off for selectable
  fields. If you virtualize, convey the full set with
  `aria-setsize`/`aria-posinset` on the list-role row elements (not the
  checkbox), or state the total count in text.
