...
<Alert variant="warning">Use caution when using the <code>isTextVisuallyHidden</code> prop; it should only be used in rare circumstances when the purpose of the toggle is conveyed in another way. While screen readers will announce the label when the toggle gains focus, it is still best to have a visual label. This is especially helpful for individuals with cognitive disabilities and users who have fine motor control limitations.</Alert>
