...
<Alert variant="info">Due to the nature of SkipLinks the best way to view the examples is to click "EDIT IN CODESANDBOX". When codesandbox loads click "open in new window" and hit tab to give focus to the SkipLink.</Alert>
  ...
  return (
    <nav>
      {[...Array(length).keys()].map(index => (
        <div key={index}>
          <Hyperlink to={`#${index}`}>Link {index +1}</Hyperlink>
        </div>
      ))}
    </nav>
  );
