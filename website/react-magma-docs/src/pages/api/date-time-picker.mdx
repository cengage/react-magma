...
The `timezone` prop expects a valid timezone identifier supported by [date-fns](https://date-fns.org/v2.0.0-alpha.27/docs/Time-Zones), such as `"America/New_York"`, `"Europe/London"`, or `"Asia/Tokyo"`.
The component validates the value against the list provided by date-fns and will not accept invalid or unsupported identifiers.
...
export function Example() {
  const [data, setData] = React.useState<{
    ...
    timezone?: string;
  }>({ day: null, event: null, timezone: undefined });
  ...
  function handleDateChange(
    ...
    setData({ day, event, timezone });
  }

  return (
    <div>
      <DateTimePicker
        labelText="Date and Time with Timezone"
      ...
      <pre>
        {JSON.stringify(
          {
            day: data.day ? data.day.toString() : null,
            timezone: data.timezone,
            event: data.event ? '[SyntheticEvent]' : null,
          },
          null,
          2
        )}
...
export function Example() {
  ...
  function hasErrorMessage() {
    const convertedMinDate = getDateFromString(minDate) ?? undefined;
    const convertedMaxDate = getDateFromString(maxDate) ?? undefined;
...
export function Example() {
  ...
  function handleDateChange(newChosenDate: Date | null) {
    let newDate: Date | undefined = newChosenDate ?? undefined;
...
export function Example() {
  ...
  function handleInputBlur(event: React.FocusEvent<Element>) {
    updateIsFocused(false);
  }

  function handleInputFocus(event: React.FocusEvent<Element>) {
