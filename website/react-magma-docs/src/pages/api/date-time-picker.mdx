...
The `timezone` prop expects a valid time zone identifier supported by [date-fns](https://date-fns.org/v2.0.0-alpha.27/docs/Time-Zones), such as `"America/New_York"`, `"Europe/London"`, or `"Asia/Tokyo"`.
...
export function Example() {
  return (
    <DateTimePicker
      labelText="Date and Time with Time Zone"
      value={new Date(2025, 1, 0, 8, 20)}
