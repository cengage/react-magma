...
```typescript
import React from 'react';
import { Datagrid, Card, TableRowColor } from 'react-magma-dom';
...
Pagination controls are put on the `Datagrid` component by default. You can turn off pagination by passing the `withoutPagination` prop as `true`.
...
export function Example() {
  ...
  return <Datagrid columns={columns} rows={rows} withoutPagination />;
}
```
