...
An alternative presentation of `Pagination` is availble with the `simple` prop. This converts the existing structure into a dropdown format, while still utilizing previous and next navigation buttons.
...
export function Example() {
  return <Pagination count={4} type="simple" />;
}
```
