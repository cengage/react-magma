...
import { Combobox, ThemeInterface } from 'react-magma-dom';

const typedStyled = styled as CreateStyled<ThemeInterface>;

export function Example() {
  const CustomStyledItem = typedStyled.li(props => ({
    alignSelf: 'center',
  ...
  const ContainerSpan = typedStyled.span(props => ({
    display: 'flex',
    alignItems: 'center',
  }));

  const Hex = typedStyled.span(props => ({
  ...
  const Description = typedStyled.div(props => ({
    fontSize: '12px',
