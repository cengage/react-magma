...
- **`radiogroup`** — a single-choice selection that is semantically a radio group. Pair with `exclusive`. Announced as a radio group containing radio buttons.
...
Use `role="radiogroup"` with `exclusive` for a single-choice selection that should be announced as a radio group.
