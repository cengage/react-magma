...
<Alert variant="warning" style={{ padding: '4px' }}>
  When multiple `NavTabs` components are used on the same page, each one should
  have its own accessible name. You can provide this by using `aria-label`,
  ensuring that every `NavTabs` instance has a unique label.
</Alert>
...
import { Card, CardBody, NavTab, NavTabs, magma } from 'react-magma-dom';
