...
<Alert>
  ...
  <inlineCode className="inverse">aria-label</inlineCode> property that
  accurately describes the function of the button, so that screen-reader users
  will know the button's purpose.
</Alert>
<Alert>
  For icon buttons with text, an{' '}
  <inlineCode className="inverse">aria-label</inlineCode> is not needed, as the
...
```tsx
import React from 'react';
import { IconButton, ButtonVariant, ButtonGroup } from 'react-magma-dom';
import {
  PriorityHighIcon,
  NotificationsIcon,
} from 'react-magma-icons';
