...
<Alert variant="info">
  For displaying informational content or filters, consider using the{' '}
  <Link to="/api/popover">Popover</Link> component instead of Dropdown.
</Alert>
...
export function Example() {
  ...
  return (
    <Popover width="300px">
      <PopoverTrigger aria-label="Open popover">{`Popover Trigger (${checkedItems.length})`}</PopoverTrigger>
      <PopoverContent>
        <PopoverHeader>
          <span>Checkbox group label</span>
        </PopoverHeader>
        <form
          style={{
            color: magma.colors.neutral,
...
### Popover with Informational Content
...
export function Example() {
  return (
    <Popover width="300px">
      <PopoverTrigger>Informational Popover</PopoverTrigger>
      <PopoverContent>
        <Paragraph noMargins>
          In this activity you must achieve 80% or higher to receive credit
        </Paragraph>
