...
The `DropdownContent` component displays menu items (`DropdownMenuItem`) when the button is clicked.
...
The `DropdownSplitButton` supports a `leadingIcon` prop that allows you to display an icon before the button text. This icon appears on the main action button (left side of the split), while the dropdown caret remains on the toggle button (right side).
...
_Deprecation note:_ Dropdowns now support viewport detection, so there is no need to use the `dropDirection` prop. The dropdown will always open where there is enough space, and the arrow will always face down.
...
_Deprecation note:_ Dropdowns now support viewport detection, so there is no need to use the `alignment` prop.
...
The Dropdown component is technically capable of displaying content other than menus. But for all non-menu content, please use the <Link to="/api/popover">Popover component</Link>. The Popover has been specifically designed for the usability and accessibility needs of such content.
