...
By setting the `hoverable` prop to true, the popover will remain open while the pointer is over either the trigger or the content. Note that this feature only works when the popover content contains no interactive elements (buttons, inputs, links, etc.). If interactive elements are detected, the popover will automatically use click behavior instead.
...
export function Example() {
  ...
  function handleClose(event: React.SyntheticEvent) {
    popoverApiRef.current && popoverApiRef.current.closePopoverManually(event);
  }

  function handleOpen(event: React.SyntheticEvent) {
    popoverApiRef.current && popoverApiRef.current.openPopoverManually(event);
  }
...
export function Example() {
  ...
  return (
    ...
    >
      <Popover focusTrap width="target" apiRef={popoverApiRef}>
        <PopoverTrigger aria-label="Open popover">
