...
<LeadParagraph>
  Magma exposes an I18nContext object and an object named defaultI18n that
...
- `locale` is a `date-fns` locale object. You can [see the available objects here](https://github.com/date-fns/date-fns/tree/master/src/locale).
  ...
  <Alert variant="warning">The locale in the default object is null to prevent the date-fns library from being bundled in your project when you are not using any of its functionality. When you use a component that depends on the date-fns library, Magma will default your locale to enUS.</Alert>
