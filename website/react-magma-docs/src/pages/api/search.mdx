...
<PageContent componentName="search" type="api">

<LeadParagraph>
  The Search component is the standardized use of the{' '}
  <Link to="/api/input/">Input</Link> component with type search.
</LeadParagraph>
...
import { Search, Card, CardBody, magma } from 'react-magma-dom';
...
export function Example() {
  ...
  return <Search onSearch={handleSearch} isPredictive isClearable />;
}
```
