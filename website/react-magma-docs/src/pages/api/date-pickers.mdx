...
After a date has been selected the user can programmatically clear the input field and the `Chosen Date:` field via `isClearable` X button or `Clear Date` button.
