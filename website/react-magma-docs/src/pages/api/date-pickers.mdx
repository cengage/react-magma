...
export function Example() {
  ...
  function handleDateChange(newChosenDate: Date | null) {
    setChosenDate(newChosenDate ?? undefined);
  }
...
export function Example() {
  return <DatePicker labelText="Error *" errorMessage="You made an error" />;
}
```
...
By default, the `DatePicker` uses a single input field.
You can change this to use separate fields for month, day, and year by using the `isDateFieldInput` prop and setting it to true.
...
export function Example() {
  ...
  function handleChange(
    value: string,
    event: React.SyntheticEvent | React.ChangeEvent<Element>
  ) {
