...
Some selects may need additional context related to the user by way of a help link. In such a case, the usage of `additionalContent` within the `Select` component enables this feature.

By default, the icon will display inline with a `Select` label to the right. When `labelPosition` is set to `left`, the label, select, and icon display inline. If no label is required, use `visuallyHidden` and the same inline layout will remain.
