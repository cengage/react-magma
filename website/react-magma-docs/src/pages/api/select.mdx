...
export function Example() {
  const [selectedItem, updateSelectedItem] = React.useState(null as any);
...
export function Example() {
  ...
  return (
    <>
      <Select
        additionalContent={tooltipContent}
...
export function Example() {
  const CustomStyledItem = styled.li<{ isFocused?: boolean }>(props => ({
    alignSelf: 'center',
