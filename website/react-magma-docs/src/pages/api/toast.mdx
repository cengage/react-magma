...
`Toast` components use the <Link to="/api/alert">Alert component</Link> internally for its content and styles. Any props that can be passed to an `Alert` can be passed to a `Toast` component. Note that Toast messages can only be 1 or 2 lines long.
