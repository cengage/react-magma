---
pageTitle: Grid API
title: Grid
props:
  - GridProps
  - Grid
  - GridItem
---

<DocsHeading type="api">Grid</DocsHeading>

CSS Grid React Component

## Basic Usage

The Grid component is simply a container that has `display: grid;` on it. All of the CSS Grid options can be added via props.

Child elements can be added as they are, or inside of `<GridItem></GridItem>`. `GridItem` accepts props that allow it to be styled to span columns or rows, and be part of a grid area.

```typescript
import React from 'react';
import { styled } from '@emotion/styled';
import { Grid, GridProps, GridItem } from 'react-magma-dom';

export function Example() {
  return (
    <Grid
      gridColumns='repeat(6, 1fr)'
      gridGap='1em'
      gridJustifyContent='center'
      gridAlignItems='center'
    >
      <GridItem
        gridRowSpan='1 / 3'
        gridColSpan='1 / 3'
        style={{ border: '1px solid red', textAlign: 'center', padding: '1em' }}
      >
        gridRowSpan='1 / 3' gridColSpan='1 / 3'
      </GridItem>
      <GridItem
        gridColSpan='3 / 5'
        style={{ border: '1px solid red', textAlign: 'center', padding: '1em' }}
      >
        gridColSpan='3 / 5'
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
      <GridItem
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        GridItem
      </GridItem>
    </Grid>
  );
}
```

## Accessibility

While CSS Grid allows the re-ordering of elements visually, it does _not_ change the tab order or how elements are read via screen reader.
The correct way is to order elements in the DOM in the way they should be read. `<Grid />` and `<GridItem />` both accept the `as` prop to allow for semantic HTML.

```typescript
import React from 'react';
import { styled } from '@emotion/styled';
import { Grid, GridProps, GridItem } from 'react-magma-dom';

export function Example() {
  return (
    <Grid
      gridColumns='repeat(4, 1fr)'
      gridGap='1em'
      gridJustifyContent='center'
      gridAlignItems='center'
    >
      <GridItem
        gridRowSpan='1 / 4'
        gridColSpan='1 / 1'
        as="nav"
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        Nav
      </GridItem>
      <GridItem
        gridColSpan='2 / 4'
        gridRowSpan='1 / 3'
        as="section"
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        Section
      </GridItem>
      <GridItem
        as="aside"
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        Aside
      </GridItem>
      <GridItem
        as="figure"
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
          margin: '0',
        }}
      >
        Figure
      </GridItem>
      <GridItem
        gridColSpan='2 / 5'
        as="footer"
        style={{
          border: '1px solid black',
          textAlign: 'center',
          padding: '1em',
        }}
      >
        Footer
      </GridItem>
    </Grid>
  );
}
```

## Responsiveness

Grids can be placed within media queries, nested inside other grids, intrinsically resized, and structured with flexible units. For these reasons, we did not explicitly declare a way to make a responsive grid within Magma.

## More on CSS Grid

[The Complete Guide to Grid](https://css-tricks.com/snippets/css/complete-guide-grid/) on [CSS-Tricks.com](https://css-tricks.com/) explains everything about grid in depth.

# Grid Props

<GridProps />

**Any other props supplied will be provided to the wrapping `div` element.**
