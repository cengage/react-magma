...
The `Flex` component has two types of layout, container and item, which is set using the props `isContainer` and `isItem`.
...
import { Flex, FlexProps, Card, CardBody } from 'react-magma-dom';
...
```tsx
import React from 'react';
import { Flex, FlexProps, Card } from 'react-magma-dom';
...
```tsx
import React from 'react';
import { Flex, Card, CardBody } from 'react-magma-dom';
