...
While Magma provides the error styling, it is up to the consumer app to handle the validation. We recommend using a `ref` on the input for accessibility.
...
import { Card, CardBody, Input, magma } from 'react-magma-dom';
