...
If using `left`, an additional prop `labelWidth` takes a number value which sets a width for the label based in percentage. This is useful when multiple inputs are shown in a column display to give a uniform appearance.
