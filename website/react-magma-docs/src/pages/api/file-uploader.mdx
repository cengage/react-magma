...
The files added to the file uploader will be sent processed when by the `onSendFile` function passed
...
Here are some examples of the file uploader. Omitted from the following is uploading the file to some service.
...
Since the file uploader uses the [FileReader API](https://developer.mozilla.org/en-US/docs/Web/API/FileReader) the possibilities of what can be done with a file are endless.
...
export function Example() {
  ...
  const onSendFile = (props: OnSendFileProps) => {
    ...
    const reader = new FileReader();
    reader.onload = function(evt) {
      const [columns, rows] = evt && evt.target && evt.target.result && csvToJson(evt.target.result.toString()) || []
      setColumns(columns);
      setFile(rows);
      onFinish && onFinish({file})
    };
