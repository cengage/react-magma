...
<Alert variant="warning">To improve accessibility, when using the <code>isSortable</code>{' '} prop, it is recommended to also include the <code>header</code>{' '}prop with a string value that indicates the name of the column. This will be used in the <code>aria-label</code> for the sort button to provide more context for screen reader users.</Alert>
