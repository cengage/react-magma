...
export function Example() {
  ...
  return (
    <RadioGroup labelText="Nodes for labels" name="nodesForLabelsGroup">
      <Radio
...
export function Example() {
  ...
  const [radioGroupVal, setRadioGroupVal] = React.useState<string | undefined>(
    undefined
  );
  ...
  function handleSelectOne() {
    if (!radioRef.current) {
      setRadioGroupVal(undefined);
      setAnnouncement('');
    } else if (radioRef.current && radioRef.current.checked) {
      setRadioGroupVal('');
  ...
  function handleFocusOne() {
    radioRef.current && radioRef.current.focus();
  }
