...
import { DownshiftSelect, Combobox } from 'react-magma-dom'
import { Select as LegacySelect } from 'react-magma-legacy-selects'
...
Each item in the `items` array can either be a `string` or an `object` with the shape `{item: string; label: string;}`, as shown in the <Link to="/api/downshift-select/#basic_usage">Basic Usage examples</Link>.
In the new component, you may also pass in a <Link to="/api/downshift-select/#custom_items">custom shape for your items</Link>, by passing in an additional prop named `itemToString`,
...
To create the equivalent of `LegacyAsyncCreatableSelect` you can pass in the new options to a <Link to="/api/combobox/#async">Combobox with the isLoading prop</Link> to indicate that the new options are loading.

To create the equivalent of the `LegacyAsyncSelect`, you would use the `Combobox` component in the same manner with the prop <Link to="/api/combobox/#disable_create_item">disableCreateItem</Link> in addition to `isLoading`.
...
See the docs for information on <Link to="/api/downshift-select/#specific_state_changes">Specific State Changes</Link> and <Link to="/api/downshift-select/#generic_state_change">Generic State Change</Link>.

## Accessibility

New to the upgraded `Select` and `Combobox` components, are some new props to allow for <Link to="/api/downshift-select/#accessibility">customization of the screen-reader accessible messages</Link>.
...
New to the upgraded `Select` and `Combobox` components, is the ability to add <Link to="/api/downshift-select/#custom_styles">custom styles</Link> by passing CSS properties.
