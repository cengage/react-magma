...
`react-select` is no longer needed as a `peerDependency` if you are soley using the new components. Remove `react-select` from your `dependencies`.
...
To create the equivalent of the `LegacyAsyncSelect`, you would use the <Link to="/api/combobox/#async">Async Combobox</Link> component iwith the prop <Link to="/api/combobox/#disable_create_item">disableCreateItem</Link> in addition to `isLoading`.
