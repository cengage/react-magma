...
import { Combobox, ComboboxStateChangeTypes, Alert, I18nContext, defaultI18n } from 'react-magma-dom'
...
<Alert variant="info">
  When using any of the <code>initial*</code> props be aware that passing in the
  controlled version of that prop will overwrite the initial version.
  <br />
  <br />
  <strong>Ex: </strong>
  <code>selectedItem</code> takes precedence over{' '}
  <code>initialSelectedItem</code>
</Alert>
  ...
  () => {
    return (<I18nContext.Provider value={{
      ...
      combobox: {
        ...
        createLabel: inputValue => `Custom Create "${inputValue}"`
      },
      multiCombobox: {
