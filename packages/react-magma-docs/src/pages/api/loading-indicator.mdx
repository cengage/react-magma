...
**Any other props supplied will be provided to the internal element. The internal element will be either a `div` when using the `ProgressBar` or a `span` when using the `Spinner`. This is controlled by the passed in `type` prop.**
