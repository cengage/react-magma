...
```.jsx
  ...
  function toggleError(){
   ...
   return( <>
      <Input
          errorMessage={hasError ? 'Please correct this error': null}
          helperMessage='Helper text'
          labelText='Error Message' />
      <Button onClick={toggleError}>Toggle Error Message</Button>
    </>)
  }
```
...
When there isn't a visible label, the placeholder text should match the invisible label word for word. This is so speech input users can operate the control with voice commands.
...
```.jsx
  <Input
      ...
      errorMessage="Styled errror message"
