...
An `icon` may be included with the input, by passing in the `icon` prop. The `iconPosition` position prop is optional and takes the values `left` and `right`, with `left` being the default.

TODO:  Input with clickable icon
