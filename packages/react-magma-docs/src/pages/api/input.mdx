...
Functionality can be added to the icon by using the `iconOnClick` and `iconOnKeyDown` props. All clickable input icons will appear on the right.
...
```.jsx
    <div>
      ...
      <Input
        ...
        iconOnClick={()=>{alert('Icon clicked')}}
        placeholder='Type in here'
        icon={<CalendarIcon />} />
    </div>
```
...
<SimplePropsTable
  props={{
    ...
    type: {
      type: {
        name: 'enum',
        options: ['InputType.email', 'InputType.number', 'InputType.password', 'InputType.text']
      },
      required: false,
