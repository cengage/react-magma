...
The `NavTabs` component is a navigation element that is styled similarly to the `Tabs` component. While they appear visually similar,
...
<SimplePropsTable
  propertyValues={{
    ...
    component: {
      ...
      required: false,
      description:
        'The prop for custom component instead of `a` in NavTab. When using this prop the `children` of the component is not used.',
    },
    testId: {
      type: {
        name: 'string',
...
export default props => (
  <ScopeableLayout
    {...props}
    components={{ NavTabs, NavTab, GraduationCapIcon, BellIcon, AndroidIcon }}
  />
)
