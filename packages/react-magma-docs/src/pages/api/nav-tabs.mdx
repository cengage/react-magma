...
```.jsx
  <NavTabs aria-label="Sample Scrollable Nav Tabs" backgroundColor='#F7F7F7'>
    <NavTab>First item with a longer text label</NavTab>
    <NavTab>Second item</NavTab>
    <NavTab>Third item</NavTab>
    <NavTab>Fourth item</NavTab>
    <NavTab>Fifth item</NavTab>
    <NavTab>Sixth item</NavTab>
    <NavTab>Seventh item</NavTab>
    <NavTab>Eighth item</NavTab>
  </NavTabs>
```
...
<SimplePropsTable
  propertyValues={{
    ...
    hasScrollButtons: {
      ...
      required: false,
      description:
        'The prop for turning on/off buttons for scrolling NavTabs component'
    },
    testId: {
      type: {
        name: 'string'
...
<SimplePropsTable
  propertyValues={{
    ...
    component: {
      ...
      required: false,
      description:
        'The prop for custom component instead of `a` in NavTab. When using this prop the `children` of the component and the `to` prop are not used.'
    },
    testId: {
      type: {
        name: 'string'
