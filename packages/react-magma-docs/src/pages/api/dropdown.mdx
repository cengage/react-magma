...
you can pass in a function named `onBeforeShiftFocus` in which you can set `preventMagmaFocus` to true on the `event`
...
```.jsx
  (props) => {
    ...
    return (<>
      ...
      <Dropdown onBeforeShiftFocus={event => event.preventMagmaFocus = true}>
