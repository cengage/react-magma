...
Out of the box, the `Select` component uses React Magma-styled iconography for the button used to clear the selection.
However, this can be overridden by providing custom components to the `components` prop, in an object containing a `ClearIndicator`.
