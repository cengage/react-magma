...
The React Select component by Jed Watson currently does _not_ support the `aria-describedby` attribute.  A few issues have been logged, 
but the workarounds suggested have not worked.  As a result, the error messages and helper messages for Select components are _not accessible_.

* https://github.com/JedWatson/react-select/issues/1787
* https://github.com/JedWatson/react-select/issues/1570
* https://github.com/JedWatson/react-select/pull/3273
...
*NOTE:*  This feature is currently *not accessible*.  The error message will appear on screen, but will not be associated with the select field for screen readers.
Please avoid using select boxes in ways that may trigger an error.  For example, if a field is required, provide a default value.
...
*NOTE:*  This feature is currently *not accessible*.  The helper message will appear on screen, but will not be associated with the select field for screen readers.
