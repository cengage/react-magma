...
import { Select, SelectStateChangeTypes, Alert, I18nContext, defaultI18n } from 'react-magma-dom'
...
Some of the internationalization overrides use functions to allow you to have the internal data that the
default labels are using.
...
```.jsx
  () => {
    return (<I18nContext.Provider value={{
      ...
      multiSelect: {
        selectedItemButtonAriaLabel: (selectedItem) =>
