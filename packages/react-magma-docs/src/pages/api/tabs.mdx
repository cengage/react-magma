...
If a `Tab` is `disabled` using the `isDisabled` prop, it will not be actionable. It will still appear in the menu, but will be styled to appear greyed out.

```.jsx
    <TabsContainer>
      <Tabs aria-label="Sample With Disabled Tabs" backgroundColor='#F7F7F7'>
        <Tab>Main page</Tab>
        <Tab isDisabled>FAQ</Tab>
        <Tab>About us</Tab>
      </Tabs>
