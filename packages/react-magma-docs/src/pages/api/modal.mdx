...
The close button can be hidden by using the `isCloseButtonHidden` prop. If this prop is used, it is best to provide another way to close the modal.
  ...
  (props) => {
    ...
    return (<>
      <Modal header="Modal Title" onClose={closeModal} isOpen={showModal}>
        Blah
        <Button onClick={customCloseModal}>Show Modal</Button>
      </Modal>
