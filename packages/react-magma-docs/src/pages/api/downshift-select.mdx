...
Set inital state using the `initialIsOpen`, `initalSelectedItem`, and `initialHighlightedIndex` props.

```.jsx
  () => {
    return <>
      <DownshiftSelect
        id="initialSelectedSelectId"
        name="initialSelected"
        labelText="Initial Selected"
        items={['Red', 'Blue', 'Green']}
        initialIsOpen
...
These props are `defaultIsOpen`, `defaultSelectedItem`, and `defaultHighlightedIndex`.
  ...
  () => {
    return <>
      <DownshiftSelect
        ...
        items={['Red', 'Blue', 'Green']}
        getA11yStatusMessage={({highlightedItem}) => `${highlightedItem} is highlighted`}
        getA11ySelectionMessage={({selectedItem}) => `${selectedItem} is now selected`}
      />
    </>
  }
```
