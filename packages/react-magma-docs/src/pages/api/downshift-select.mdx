...
import { DownshiftSelect, SelectStateChangeTypes, Alert } from 'react-magma-dom'
...
If you are using a custom items format see the [Custom Items example](/api/downshift-select/#custom_items).
...
<Alert variant="info">
  When using any of the <code>initial*</code> props
  be aware that passing in the controlled version of that prop will overwrite the initial version.
  <br />
  <br />
  <strong>Ex: </strong><code>selectedItem</code> takes precedence over <code>initialSelectedItem</code>
</Alert>
...
<Alert variant="warning">
  Using <code style={{background:'rgba(0,0,0,0.1)', border: 0}}>onStateChange</code> and controlling every
  piece of <code style={{background:'rgba(0,0,0,0.1)', border: 0}}>state</code> will result in losing most of the functionality
  that has been built in to this component. Only use this feature as a last resort.
</Alert>
...
to change the state. You can see the full list of the types in the [`stateChangeTypes`](/api/downshift-select/#state_change_types) section.
...
<Alert variant="info">
  ...
  <br />(eg: <code style={{background: 'rgba(255, 255, 255, 0.1)', border: '0px', color: '#fff'}}>ComboboxStateChangeTypes.InputKeyDownArrowDown = '__input_keydown_arrow_down__'</code>).
  However, in the production environment the types equate to numbers
  <br />(eg: <code style={{background: 'rgba(255, 255, 255, 0.1)', border: '0px', color: '#fff'}}>ComboboxStateChangeTypes.InputKeyDownArrowDown = 0</code>).
</Alert>
