...
Toasts are used to display non-critical information to users.

## Basic Usage

Variants for toasts include `info`, `success`, `warning`, and `danger`, with `info` being the default value.

It is recommended that `warning` and `danger` toasts do not auto-dismiss, so that users have time to process the information in them.
