...
```tsx codesandbox=magma
import React from 'react';
import { Button, ButtonColor, Toast, AlertVariant } from 'react-magma-dom';
...
export function Example() {
  ...
  return (
    <ToastsContainer>
      <Announce>
        <Button onClick={() => setShowToast(true)} color={ButtonColor.success}>
          Show Toast
        </Button>
        {showToast && (
          <Toast
        ...
        {showToast2 && (
          <Toast id="toast2" onDismiss={() => setShowToast2(false)}>
            This is the second toast which is longer than either toast one or
            three
          </Toast>
        )}

        <Button onClick={() => setShowToast3(true)} color={ButtonColor.danger}>
          Show Toast 3
        </Button>
        {showToast3 && (
          <Toast
