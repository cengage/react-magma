...
The property tables that we use are generated automatically, but require some wiring up. The interface in the typescript file must be annotated with typedoc comments and then the needed props get added to the front matter in the `mdx` file and there is a magic component for the property table which matches the props in the frontmatter. For example:
