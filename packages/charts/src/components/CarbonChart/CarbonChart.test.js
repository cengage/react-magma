...
describe('CarbonChart', () => {
  ...
  describe('Legend accessibility wrap', () => {
    ...
    it('uses translated legend selection announcements from I18nContext', () => {
      const i18nValue = {
        ...defaultI18n,
        charts: {
          ...defaultI18n.charts,
          toolbar: {
            legendAllSelected: 'Tous les éléments sont sélectionnés',
            legendOnlySelected: 'Seul {label} est sélectionné',
          },
        },
      };
      ...
      const wrapper = getByTestId(testId);
      const checkboxes = wrapper.querySelectorAll('.legend-item .checkbox');
      const selectedLabel = wrapper
        .querySelector('.legend-item p')
        .textContent.trim();
      ...
      act(() => {
        checkboxes.forEach((checkbox, index) => {
          checkbox.setAttribute('aria-checked', index === 0 ? 'true' : 'false');
        });
        mutationObserverCallback();
      });

      expect(
        screen.getByText(`Seul ${selectedLabel} est sélectionné`)
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText('Tous les éléments sont sélectionnés')
      ).toBeInTheDocument();
    });
  });
});
