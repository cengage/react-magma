...
describe('TreeView', () => {
  ...
  describe('click anywhere on a folder toggles expand/collapse in selectable.off', () => {
    it('expands a folder when clicking the label in selectable=off mode', async () => {
      const { getByTestId } = render(
      ...
      await userEvent.click(getByTestId('parent-label'));

      expect(parent).toHaveAttribute('aria-expanded', 'true');
    });

    it('collapses a folder when clicking the label again in selectable=off mode', async () => {
      const { getByTestId } = render(
        <TreeView
          ariaLabel="TreeView"
          selectable={TreeViewSelectable.off}
    ...
    it('expands a folder when clicking the item wrapper in selectable=off mode', async () => {
      const { getByTestId } = render(
        <TreeView ariaLabel="TreeView" selectable={TreeViewSelectable.off}>
          <TreeItem label="Parent" itemId="parent" testId="parent">
            <TreeItem label="Child" itemId="child" testId="child" />
      ...
      await userEvent.click(getByTestId('parent-itemwrapper'));

      expect(parent).toHaveAttribute('aria-expanded', 'true');
    });

    it('fires onExpandedChange when clicking a folder in selectable=off mode', async () => {
      const onExpandedChange = jest.fn();
    ...
    it('does nothing when clicking a leaf item in selectable=off mode', async () => {
      const onExpandedChange = jest.fn();
      ...
      await userEvent.click(getByTestId('leaf-label'));

      expect(leaf).not.toHaveAttribute('aria-expanded');
      expect(onExpandedChange).not.toHaveBeenCalled();
    });

    it('does not expand a disabled folder when clicked in selectable=off mode', async () => {
      const onExpandedChange = jest.fn();
    ...
    it('does not toggle expansion when clicking a folder in single-select mode with selectParents=true (default)', async () => {
      const onExpandedChange = jest.fn();
      const onSelectedItemChange = jest.fn();
    ...
    it('does not toggle expansion when clicking a folder row in multi-select mode', async () => {
      const onExpandedChange = jest.fn();
