...
- 8f6c89176: fix(Table & Datagrid): Fix screen reader announcements for sortable and selectable tables (A11Y-529, A11Y-530, A11Y-531, A11Y-532).
...
- 53360da2d: fix(Select, Multiselect): Separate focus and hover states and auto-focus first item for NVDA support
...
- de4bd2938: fix(Tooltip & Loading Indicator): Force visibility tooltip content when user hover it. Fix announcing `Loading` twice for Loading indicator.
...
- f7ec34cfc: fix(Dropdown): Remove `aria-label` from DropdownMenuItem icon. Add `role=menu` to DropdownContent for DropdownMenuNavItem children. Fix docs examples.
...
- a841b5629: fix(DatePicker): Fix focusing input field after clearing date and when `isDateFieldInput=true`.
...
- 33509ff0c: fix(Pagination): Add `aria-hidden=true` for ellipsis.
...
- 3ab0f7067: fix(Select, Multiselect, Combobox & Multicombobox): Fix navigation throughth the disabled items.
...
- ff0c545ef: fix(Tag): Add a new `aria-label` attribute to the component to improve accessibility.
